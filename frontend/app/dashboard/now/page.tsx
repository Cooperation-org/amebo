...
/**
 * Now — one screen: what is top for this person, then every live goal and
 * where it stands. Golda 2026-09-06: "the agent deals with the complexity and
...
export default function NowPage() {
  ...
  const rows = [...(top.data?.pinned ?? []), ...(top.data?.live ?? [])];
  const gs = goals.data ?? [];
  const counts = {
  ...
  return (
    <div className="space-y-8">
      <section className="space-y-2">
        ...
        {top.data && top.data.live_total > rows.length && (
          <Link href="/dashboard/list" className="block pt-1 text-xs text-gray-500 hover:underline">
