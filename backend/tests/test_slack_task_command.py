...
def test_full_command():
    payload, err = parse_task_command(
        "amebo Ship the badge embed due:2099-06-20 assign:golda cash:50"
    )
    assert err is None
    assert payload == {
        "project": "amebo",
...
def test_minimal_project_subject_due():
    payload, err = parse_task_command("amebo Fix the thing due:2099-07-01")
    assert err is None
    assert payload == {
        "project": "amebo", "subject": "Fix the thing", "due_date": "2099-07-01",
    }


def test_keys_can_precede_rest_of_subject():
    # key:value tokens may appear anywhere after the project.
    payload, err = parse_task_command("bd due:2099-06-30 Call the prospect back assign:peter")
    assert err is None
    assert payload["subject"] == "Call the prospect back"
    assert payload["due_date"] == "2099-06-30"
...
def test_missing_subject():
    payload, err = parse_task_command("amebo due:2099-06-20")
    assert payload is None
    assert "Missing task subject" in err


def test_cash_must_be_number():
    payload, err = parse_task_command("amebo Do it due:2099-06-20 cash:lots")
    assert payload is None
